		...
		for iface in interfaces:
			for signal in iface.signals:
				...
				def EmitSignal(iface, signal):
					return lambda *args: self.SignalEmitted(iface.name, signal.name, Variant("(" + "".join(s.signature for s in signal.args) + ")", args))
		...
		if "org.freedesktop.DBus.Properties" not in (iface.name for iface in interfaces):
			try:
				def onPropertiesChanged(iface, changed, invalidated):
					changed = {key: Variant(self.readable_properties[iface + "." + key], val) for key, val in changed.items()}
					args = Variant("(sa{sv}as)", (iface, changed, invalidated))
	...
	def call_method(self, connection, sender, object_path, interface_name, method_name, parameters, invocation):
		try:
			...
			if len(outargs) == 0:
				...
				invocation.return_value(Variant("(" + "".join(outargs) + ")", (result,)))
			else:
				invocation.return_value(Variant("(" + "".join(outargs) + ")", result))
	...
	def GetAll(self, interface_name):
		...
		for name, type in self.readable_properties.items():
			...
			if ns == interface_name:
				ret[local] = Variant(type, getattr(self.object, local))
		return ret
